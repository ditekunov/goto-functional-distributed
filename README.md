## Играемся тут с функциональщиной и Скалой в частности

### Всем привет, тут у нас будет общий репозиторий с полезными ссылками, примерами кода и заданиями

#### Day 1

##### 0)  Все связанное с окружением:

[`Питон_скачать_онлайн_без_регистрации`](https://www.python.org/downloads/) 

[`Онлайн версия для ленивых`](https://www.tutorialspoint.com/execute_python_online.php) 

[`Пайчарм_скачать_онлайн_без_регистрации`](https://www.jetbrains.com/pycharm/download/#section=windows) 

[`Юпитерский ноутбук`](http://jupyter.org) 

##### 1)  Ссылки на питонячие ресурсы:

[`Чайничный ресурс`](https://pythonworld.ru) 

[`Дока`](https://docs.python.org/3/) 

##### 2)  Задачи:
[`Основные задачки на ФП в питоне - 1`](python/python_basics.md)

[`Основные задачки на ФП в питоне - 2`](python/python_basics2.md)


##### 3)  Доп. задачи:

[`Сложные задачки на ФП в питоне`](python/python_advanced.md)

##### 4) Экзамен по Python:

[`Удачи!`](python/python_exam.md)


#### Day 2

[`Основные задачки на ФП в Scala - 1`](scala/scalaBasics.md)

[`Сложные задачки на ФП в Scala`](scala/scalaBasics2.md)

#### Day 3

[`Основные задачки на ФП в Scala - 2`](scala/scala3.md)

[`Сложные задачки на ФП в Scala - 2`](scala/scala3Advanced.md)

[`Оп домашка оп`](scala/scala3Home.md)

#### Day 4

[`Основные задачки на ФП в Scala - 3`](scala/scala4.md)

[`Сложные задачки на ФП в Scala - 3`](scala/scala4Advanced.md)

[`Домашка по Scala - 2`](scala/scala4Home.md)

[`Экзамен за блок по Scala`](scala/scalaExam.md)

#### Day 5

[`Проект 1`](projects/project1.md)

[`Код с пары`](web/routeExample.scala) 

#### Day 6

[`Код с пары`](web/actorExample.scala)

#### Day 7

[`Экзамен за блок WEB`](web/exam.md)

#### Day 8

[`Задачи на персистентные алгоритмы - 1`](algo/algo1.md)

#### Day 9

#### Day 10

#### Day 11
[`Экзамен по курсу КРС`](final/exam.md)
